...
1. **TabPFN (nauczyciel)** — tabular foundation model. Uczy się mapowania
   `widmo → usterka` z małego `val.csv` (40 silników). Na T4 w Colabie
   trzyma się w pamięci GPU; na CPU też wstaje, tylko wolniej.
...
2. Wgraj `val.csv`, `train.csv`, `test.csv` oraz `tabpfn_diagnose.py`
...
`--cv` to 5-fold GroupKFold po `engine_id` (uczciwy score, ~kilka minut na T4).
Bez `--cv` dostajesz od razu artefakty pod aplikację.
...
| `diagnose.py` | rozwiązanie 1 (RandomForest + reguły), nietknięte |
